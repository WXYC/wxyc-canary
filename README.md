...
| `lml-discogs-breaker-shed` | `GET /health` on LML — reads `discogs_breaker_state`                                                                                            | none             | LML's Discogs-saturation breaker shedding lookup traffic (library-metadata-lookup#939). **This check's own status always `pass`es** — the shed signal is carried entirely by the `DiscogsBreakerShedding` metric (0/1) and a dedicated 3-of-3 alarm (`wxyc-canary-lml-discogs-breaker-shed`), not by `CheckFailure`. `open`/`half-open` → 1; `closed`/missing/non-200/network-error → 0 (abstain on indeterminate). **Known limitation:** reading `/health` never advances the breaker's recovery state machine, so an idle overnight period after a real trip can idle-latch `open` and false-page; verify recovery via `api.discogs.com` spans resuming in Sentry, not `/health` alone — see the runbook entry below. |
...
- Per-check metrics: `CheckFailure`, `CheckSkipped`, `CheckLatency`, all dimensioned on `Check=<name>`. Plus `EnrichmentLagSeconds` from the v1 write canary, `GraphDbAgeSeconds` from `semantic-index-freshness`, and `DiscogsBreakerShedding` from `lml-discogs-breaker-shed` (all three dimensioned + dimensionless; `GraphDbAgeSeconds` is dashboard-trend-only — no alarm reads it, the freshness failure is carried by the `InfraCheckFailure` aggregate; `DiscogsBreakerShedding` has its own dedicated alarm, see below). Two dimensionless-only aggregates route failures by paging tier: `UserFacingCheckFailure` and `InfraCheckFailure` (see "Paging tier" above).
- Six alarms: `wxyc-canary-check-failure` (a **user-facing** check failed in 2 of last 3 evaluations → `AlertTopic`), `wxyc-canary-infra-degraded` (an **infra/CI** probe failed, same 2-of-3 window → low-urgency `InfraAlertTopic`), `wxyc-canary-enrichment-lag` (sentinel row took > 30 s to enrich for 3 consecutive evaluations), `wxyc-canary-lml-discogs-breaker-shed` (LML's Discogs breaker shed lookup traffic for 3 **consecutive** evaluations — 3-of-3, not 2-of-3, since a brief real trip is expected → `AlertTopic`; see the runbook's idle-tail caveat), `wxyc-canary-lambda-errors` (Lambda crashed before publishing metrics), and `wxyc-canary-mutation-4xx-surge` (a sustained surge of 4xx on Backend-Service mutation routes — a `WXYC/BackendService` metric, not a canary check → `AlertTopic`).
...
This is a **separate page alarm** from `wxyc-canary-check-failure` — the `lml-discogs-breaker-shed` check's own status never fails (see `src/checks.ts`), so this signal never shows up as a `CheckFailure`/`UserFacingCheckFailure` datapoint. It fires when the `DiscogsBreakerShedding` dimensionless metric has read `1` for **3 consecutive** 5-minute evaluations (~15 min) — LML's Discogs-saturation breaker has been shedding lookup traffic (`discogs_breaker_state` is `open` or `half-open`) and hasn't recovered on its own. The 3-of-3 window (not the shared alarm's 2-of-3) is intentional: a breaker legitimately trips OPEN for a window or two during real Discogs-side saturation, and that must not page — only a shed that holds is the "never recovers" signature.

1. Open CloudWatch → Metrics → `WXYC/Canary` → `DiscogsBreakerShedding` to confirm the sustained read, and check LML's own `/health` for the current `discogs_breaker_state` and `services.discogs_api`.
2. **Do not trust `/health` alone to judge recovery.** Reading `/health` never advances the breaker's `open -> half-open` recovery transition — only a live `/lookup` call does (library-metadata-lookup#939). After a genuine trip, an overnight or otherwise quiet period with zero live Discogs lookups leaves the breaker idle-latched `open`, and `/health` will keep reporting `open` even though the next real lookup would likely recover it immediately. Verify actual recovery via **`api.discogs.com` spans resuming in Sentry** instead: query `span.domain:*.discogs.com` (Sentry wildcard-normalizes the domain — the literal `span.domain:api.discogs.com` filter returns zero rows even while live traffic is flowing, and produced a false "still latched" reading during the 2026-07-14 post-fix verification for this exact breaker). `span.op:http.client` grouped by `span.domain` is the reliable shape.
3. If Sentry shows `api.discogs.com` spans genuinely absent while `/lookup` volume is nonzero, this is a real sustained shed — investigate LML's Discogs API quota/rate-limit state and whether the breaker's cooldown is misconfigured (see library-metadata-lookup#787 for the HALF_OPEN-latch class this replaced).
4. If Sentry shows `api.discogs.com` spans resuming (or no `/lookup` traffic at all during the window), this is the accepted idle-tail false positive — the breaker is idle-latched, not stuck. No action needed; it self-clears on the next real lookup. This is a known, accepted limitation (wxyc-canary#79) pending a follow-up LML ticket to expose a read-only lookup-rate field on `/health` so a future iteration can gate the alarm on concurrent volume.

...
| `wxyc-info-recent-entries` | `GET http://wxyc.info/playlists/recentEntries?v=2&n=50`                                                                                         | none             | The hostname the un-updatable mobile fleet has compiled in — the only probe here that asks for `wxyc.info` over plain `http://` rather than hitting `api.wxyc.org` directly. Since the 2026-09-16 DNS flip that name resolves to the Backend EC2 host, where a dedicated `server_name wxyc.info` nginx block on `:80` proxies this one route to `127.0.0.1:8080`; before it, the name resolved to Kattare and this probe measured WXYC/tubafrenzy#620's Tomcat bridge. It survives the flip because that block is **hand-edited on the box, is not in version control, and ends in a catch-all `return 410`** — lose the exact-match location and every phone gets a clean dead end while `api.wxyc.org` stays green (wxyc-canary#104). Validates the **v2 grouped shape** against the iOS decoder's real contract: `playcuts`/`breakpoints`/`talksets` are all decoded non-optionally by `Playlist.init(from:)`, so a dropped empty array blanks every phone, and every playcut is walked (iOS decodes `[Playcut].self` for the whole array, so one bad row throws the entire decode). **One lane:** every failure — timeout, any non-2xx including a `410` or the retired fail-soft `503`, a malformed body — pages via the shared alarm. |
...
### Retiring the legacy-fleet probe

`wxyc-info-recent-entries` was written with a retirement trigger of "the `wxyc.info` DNS flip, because the host under test stops existing." **That premise did not hold.** The flip landed 2026-09-16 07:32 PDT and moved the apex A record from Kattare to the Backend EC2 host; `wxyc.info` still answers, the legacy fleet still polls it, and the check kept passing across the cutover and got faster. What died was the probe's second lane — see the note under `wxyc-canary-recent-entries-latency` below — which was deleted along with `RecentEntriesUpstreamUnavailableAlarm` in wxyc-canary#104.

**The real trigger is the nginx block.** When the legacy fleet finally stops polling and the `server_name wxyc.info` block comes out of `/etc/nginx/nginx.conf`, delete the check from `src/checks.ts`, `RecentEntriesLatencyAlarm` and both parameters from `template.yaml`, and the `legacyPlaylistUrl` field from `src/types.ts`. Until then the probe is the only thing watching a hand-edited, unversioned config whose catch-all `return 410` would swallow the route silently.

The off switch, for a window where you need it quiet without a code deploy, is a redeploy with the probe disabled:

```bash
sam deploy --parameter-overrides EnableLegacyPlaylistProbe=false
```

The check then downgrades to `skipped` and the latency alarm parks at no-data/OK. That is a pause, not a retirement — it leaves live code pointed at a live surface and simply stops looking.
...
- **`wxyc-info-recent-entries`** — the hostname the un-updatable mobile fleet has compiled in stopped serving the flowsheet. Since the 2026-09-16 DNS flip this is the `server_name wxyc.info` nginx block on the Backend EC2 host, so read the status first, because it names the layer:
  - **`410`** — the block's catch-all answered, meaning the exact-match `location = /playlists/recentEntries` was dropped, reordered, or shadowed. `api.wxyc.org` will be green and nothing else on the host will notice; this probe is the only detector. That config is hand-edited and unversioned — check `/etc/nginx/nginx.conf` against `WXYC/plans/wxyc-info-front-proxy/wxyc.info.server.conf` and the dated `.bak` beside it.
  - **`502`** — nginx could not reach `127.0.0.1:8080`. Backend is down or restarting; `backend-healthcheck` should be failing too, and this is corroboration rather than a second incident.
  - **timeout, or a concurrent `wxyc-canary-recent-entries-latency` page** — Backend is _hung_ rather than down: it accepted the connection and stopped answering, so nginx holds the request against its 10 s `proxy_read_timeout` and the probe burns its whole 10 s budget. That pair firing together is the diagnostic — down gives a fast 502 and this alarm alone; hung gives both.
  - **a 200 with the wrong shape** (missing `playcuts`/`breakpoints`/`talksets`, zero playcuts, a flat array where the v2 object belongs, a playcut missing an iOS-required field) — Backend's projection changed under a client that decodes all three arrays non-optionally. Every phone throws on decode; work it as a Backend contract break.
  - A **`503`** here is Backend's own and pages like any other failure. It used to be special — see the note under `wxyc-canary-recent-entries-latency`.
...
> **This alarm used to have a sibling.** `wxyc-canary-recent-entries-upstream-unavailable` covered the case where tubafrenzy's bridge servlet could reach us but not Backend, signalled by its fail-soft `503` + `{"error":"upstream_unavailable"}`. The 2026-09-16 DNS flip took that servlet out of the path — it is the only artifact in the org that writes that body — so the lane became unreachable and both it and the alarm were deleted (wxyc-canary#104). Its last breaching datapoint was 07:25 PDT that morning, seven minutes before the flip. A `503` on this route now is Backend's own and pages through `wxyc-canary-check-failure` like any other failure. This also closes the triage-misdirection complaint in wxyc-canary#102 by removing the description it was about.

`http://wxyc.info/playlists/recentEntries` has been answering slower than **5 s** for 2 of 3 consecutive checks.

**Check whether `wxyc-canary-check-failure` is firing for the same check before reading this as early warning.** If it is, listeners _are_ seeing errors and this alarm is corroboration: the pair firing together means Backend is hung rather than down, because nginx holds the request against its 10 s `proxy_read_timeout` while the probe burns its whole 10 s budget, so the tick reports both a failure and a 10,000 ms latency. Backend being _down_ instead gives a fast `502` and the check-failure page alone. That pairing is what is left of the two-lane diagnostic the deleted sibling alarm used to provide.

If this alarm is firing **alone**, it is the early-warning half of the objective, and the threshold is half the canary's own 10 s request budget, which is in turn OkHttp's default per-socket read timeout (`WXYC-Android` `AppModule.kt` builds Retrofit without `.client(...)`). Note that's a per-hop budget, not a deadline: OkHttp's `callTimeout` defaults to disabled and `readTimeout` restarts on every successful read, and iOS's 30 s `timeoutInterval` is likewise an inactivity timer. No client enforces a wall-clock deadline, so the canary is deliberately stricter than any of them and reports early rather than late. **Re-baselined from production on 2026-09-16 (wxyc-canary#104)**, which the original note asked for — and it overturns the headroom claim it replaces. The 2026-08-05 figures (559–775 ms end-to-end, 218–251 ms warm ttfb) were taken from a laptop and put the threshold "~6.5× above the slowest observed real run." The 30 days of us-east-1 Lambda samples before the DNS flip — 8,633 points — say otherwise: a daily mean of 769–915 ms, but a worst single sample of **5,339 ms**, i.e. the threshold sits about **6% above** the observed tail, not 6.5× above it. That excursion did not page, which is the 2-of-3 window doing precisely the job it was chosen for. The tail should now fall, since the Kattare hop is gone: the first hours on the nginx path ran a 620 ms mean and a 1,043 ms max. **Do not tighten this threshold off pre-flip data** — take a fresh week on the new path first.

1. Compare the legacy hostname against the API hostname. Both now terminate on the same box and the same Backend process, so a gap between them is nginx-side, not network-side:

   ```sh
   curl -s -o /dev/null -w '%{http_code} %{time_starttransfer}\n' 'http://wxyc.info/playlists/recentEntries?v=2&n=50'
   curl -s -o /dev/null -w '%{http_code} %{time_starttransfer}\n' 'https://api.wxyc.org/playlists/recentEntries?v=2&n=50'
   ```

   **Measure warm** — both real clients hold keep-alive connections, and a cold sample pays ~130 ms of TLS + network-distance setup that no client actually pays per poll. A cold-vs-cold comparison is what produced the bogus "the proxy is faster than its own origin" reading in WXYC/tubafrenzy#626.

2. If both are slow, this is a Backend latency incident — work it there.
3. If only `wxyc.info` is slow, the fault is in its own server block or in the host: check `/var/log/nginx/wxyc.info.error.log` (that vhost logs separately on purpose) and the box's load. Note the block sets `proxy_read_timeout 10s`, so a Backend response slower than that surfaces as a `504`, not as a slow `200`.

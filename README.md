...
| `wxyc-info-recent-entries` | `GET http://wxyc.info/playlists/recentEntries?v=2&n=50`                                                                                         | none             | The tubafrenzy bridge path (WXYC/tubafrenzy#620) the mobile fleet actually polls — the only probe here that traverses DNS → Kattare Tomcat → servlet → HTTPS → AWS → Express rather than hitting `api.wxyc.org` directly. Validates the **v2 grouped shape** against the iOS decoder's real contract: `playcuts`/`breakpoints`/`talksets` are all decoded non-optionally by `Playlist.init(from:)`, so a dropped empty array blanks every phone, and every playcut is walked (iOS decodes `[Playcut].self` for the whole array, so one bad row throws the entire decode). **Two lanes:** a bridge fault (timeout, non-sentinel non-2xx, malformed body) fails and pages via the shared alarm; tubafrenzy's documented fail-soft (`503` + `{"error":"upstream_unavailable"}`) `pass`es and carries `RecentEntriesUpstreamUnavailable: 1` to its own alarm instead — so a Backend outage and a bridge fault are never the same page. **Retires with WXYC/wiki#100** (the `wxyc.info` DNS flip). |
...
### Retiring the bridge probe (WXYC/wiki#100)
...
- **Backend is also failing** (a concurrent `wxyc-canary-check-failure` naming `backend-healthcheck`) — Backend is down. This alarm is corroboration, not a second incident. Work the Backend outage; this clears on its own.
- **Backend is green** — Backend is up but unreachable _from Kattare_. That's a network-path or Kattare-side fault (egress break, DNS, TLS, the shared host wedged), and nothing else in this canary can see it. Check tubafrenzy's `wxyc-playlists.log` for `request timed out` / `HTTP connect timed out` around the alarm window, and try the bridge and the origin side by side:
...
3. If only the bridge is slow, the fault is in the Kattare hop: check Tomcat thread-pool saturation and the shared host's load.

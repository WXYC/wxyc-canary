...
- The `wxyc-canary-check-failure` alarm uses the metrics-expression form (`SUM(SEARCH(...))`) so it aggregates `CheckFailure` across every value of the `Check` dimension; a plain `MetricName` alarm queries the empty undimensioned series and sits at "no data" forever (issue #2). The `wxyc-canary-mutation-4xx-surge` alarm uses the same form against `WXYC/BackendService MutationClientError` (emitted by BS#704) to aggregate across every `Route` and `StatusCode`, threshold 20 per 5-min window for 2 windows.
